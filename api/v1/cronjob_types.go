...
type CronJobSpec struct {
	...
	// The schedule in cron format, see https://en.wikipdeia.com/wiki/Cron
	Schedule string `json:"schedule"`

	// Optional deadline in seconds for starting the job if it misses scheduled time
	// for any reasons. Missed job executions will be counted as failed ones.
	// +optional
	StartingDeadlineSeconds *int64 `json:"startingDeadlineSeconds,omitempty"`

	// Specifies how to treat concurrent  executions of a job.
	// Valid values are:
	// - "Allow" (default): Allows concurrent job to run concurrently
	// - "Forbid": forbis concurrent runs, skipping next run if previous run hasn't finished
	// - "Replace": cancels current running job and replaces it with a new one.
	// +optional
...
type CronJobStatus struct {
	...
	// Information when wa the last time the job was successfully finished
	// +optional
